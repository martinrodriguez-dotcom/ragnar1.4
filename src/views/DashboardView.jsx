...
export default function DashboardView({ clients, navigateTo, onAddClient }) {
  ...
  const [newClient, setNewClient] = useState({ name: '', email: '', plan: '', startDate: '' });

  const today = new Date();
  const currentMonthId = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}`;
  const todayId = today.toISOString().split('T')[0];

  // Cargar planes de configuración
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const snap = await getDoc(doc(db, 'settings', 'general'));
        if (snap.exists() && snap.data().plans) {
          const plansData = snap.data().plans;
          setTrainerPlans(plansData);
          if (plansData.length > 0) {
            setNewClient(prev => ({ ...prev, plan: plansData[0].name }));
          }
        }
    ...
    const fetchDashboardData = async () => {
      ...
      for (const client of clients) {
        ...
        if (paySnap.exists() && paySnap.data().status === 'paid') {
          ...
          if (diffDays <= 0) tempDebtorsList.push(client);
        }
  ...
  const handleCreateClient = (e) => {
    ...
    setNewClient({ name: '', email: '', plan: trainerPlans.length > 0 ? trainerPlans[0].name : '', startDate: '' });
  };
    ...
    <div className="max-w-6xl mx-auto animate-in fade-in pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group hover:border-yellow-400/50 transition-colors">
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><DollarSign size={64} className="text-green-500 -mr-4 -mt-4" /></div>
          <div className="flex items-center gap-3 mb-2"><div className="bg-green-500/10 p-2 rounded-lg text-green-500"><DollarSign size={20}/></div><p className="text-[10px] uppercase font-black text-zinc-500 tracking-widest">Ingresos del Mes</p></div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group hover:border-blue-500/50 transition-colors cursor-pointer" onClick={() => navigateTo('clients')}>
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Users size={64} className="text-blue-500 -mr-4 -mt-4" /></div>
          <div className="flex items-center gap-3 mb-2"><div className="bg-blue-500/10 p-2 rounded-lg text-blue-500"><Users size={20}/></div><p className="text-[10px] uppercase font-black text-zinc-500 tracking-widest">Alumnos Activos</p></div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group hover:border-yellow-400/50 transition-colors cursor-pointer" onClick={() => navigateTo('notifications')}>
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><Activity size={64} className="text-yellow-400 -mr-4 -mt-4" /></div>
          <div className="flex items-center gap-3 mb-2"><div className="bg-yellow-400/10 p-2 rounded-lg text-yellow-400"><TrendingUp size={20}/></div><p className="text-[10px] uppercase font-black text-zinc-500 tracking-widest">Entrenaron Hoy</p></div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl relative overflow-hidden group hover:border-red-500/50 transition-colors cursor-pointer" onClick={() => navigateTo('payments')}>
          <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity"><AlertTriangle size={64} className="text-red-500 -mr-4 -mt-4" /></div>
          <div className="flex items-center gap-3 mb-2"><div className="bg-red-500/10 p-2 rounded-lg text-red-500"><AlertTriangle size={20}/></div><p className="text-[10px] uppercase font-black text-zinc-500 tracking-widest">Cuotas Vencidas</p></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] p-6 shadow-xl flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2"><CheckCircle size={18} className="text-green-500"/> Finalizaron Hoy</h3>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2">
            {completedList.length > 0 ? completedList.map(client => (
                <div key={client.id} className="bg-black/50 border border-zinc-800 p-4 rounded-2xl flex justify-between items-center group cursor-pointer hover:border-zinc-700" onClick={() => navigateTo('client-detail', client)}>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-green-500/10 text-green-500 rounded-xl flex items-center justify-center font-bold">{client.name.charAt(0)}</div>
                    <div><h4 className="font-bold text-white uppercase text-sm">{client.name}</h4><p className="text-zinc-500 text-[10px] uppercase mt-0.5 line-clamp-1">"{client.performance}"</p></div>
                  </div>
                  <ChevronRight size={16} className="text-zinc-600 group-hover:text-yellow-400 transition-colors"/>
                </div>
              )) : (
              <div className="flex flex-col items-center justify-center h-full text-zinc-600 opacity-50"><Activity size={40} className="mb-3"/><p className="text-xs uppercase font-bold tracking-widest">Nadie ha finalizado aún</p></div>
            )}
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] p-6 shadow-xl flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-white font-bold uppercase tracking-widest text-sm flex items-center gap-2"><AlertTriangle size={18} className="text-red-500"/> Atención Requerida</h3>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 custom-scrollbar pr-2">
            {debtorsList.length > 0 ? debtorsList.map(client => (
                <div key={client.id} className="bg-red-500/5 border border-red-500/20 p-4 rounded-2xl flex justify-between items-center group cursor-pointer hover:border-red-500/50 transition-colors" onClick={() => navigateTo('payments')}>
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-red-500/20 text-red-500 rounded-xl flex items-center justify-center font-bold">{client.name.charAt(0)}</div>
                    <div><h4 className="font-bold text-white uppercase text-sm">{client.name}</h4><p className="text-red-400 text-[10px] uppercase font-bold mt-0.5">Cuota Vencida</p></div>
                  </div>
                  <ChevronRight size={16} className="text-red-500/50 group-hover:text-red-500 transition-colors"/>
                </div>
              )) : (
              <div className="flex flex-col items-center justify-center h-full text-zinc-600 opacity-50"><CheckCircle size={40} className="mb-3 text-green-500/50"/><p className="text-xs uppercase font-bold tracking-widest">Todos al día</p></div>
            )}
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-zinc-950 w-full max-w-md rounded-[2rem] border border-zinc-800 shadow-2xl relative overflow-hidden">
            ...
            <form onSubmit={handleCreateClient} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Email (Opcional)</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  {/* AQUÍ CARGAMOS LOS PLANES DINÁMICAMENTE */}
                  <label className="block text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">Plan Asignado</label>
                  <select required className="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-white outline-none focus:border-yellow-400 text-sm" value={newClient.plan} onChange={e => setNewClient({...newClient, plan: e.target.value})}>
