...
export default function StudentView({ clientId }) {
  ...
  // Escala de Colores RIR (del 5 al 0)
  ...
  useEffect(() => {
    if (!client) return;
    onSnapshot(collection(db, 'clients', client.id, 'sessions'), (snapshot) => {
  ...
  const handleFinishWorkout = async () => {
    if (!window.confirm('¿Finalizar entrenamiento y compartir en el Salón?')) return;
    try {
      ...
      await addDoc(collection(db, 'trainerNotifications'), {
        type: 'workout_completed', clientId, clientName: client.name, date: currentDateId, read: false, createdAt: new Date()
      });
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans flex flex-col relative overflow-x-hidden">
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-zinc-900 border border-red-500/30 rounded-3xl p-8 w-full max-w-sm text-center shadow-2xl">
            ...
            <p className="text-zinc-400 text-sm mb-8">Acceso limitado. Regulariza el pago de <span className="text-white font-bold">{date.toLocaleString('es-ES', { month: 'long' })}</span>.</p>
            <div className="bg-black/50 rounded-2xl p-5 mb-8 border border-zinc-800 text-left">
              <p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Alias Mercado Pago:</p>
              <p className="text-lg font-mono font-bold text-yellow-400 break-all">{trainerSettings.alias || 'COACH_SIN_ALIAS'}</p>
            </div>
            <button onClick={() => window.open('https://www.mercadopago.com.ar/', '_blank')} className="w-full bg-yellow-400 text-black font-black py-4 rounded-xl uppercase tracking-widest flex items-center justify-center gap-2">Pagar <ExternalLink size={18}/></button>
            <button onClick={() => setShowPaymentModal(false)} className="w-full text-zinc-500 font-bold text-xs uppercase mt-4">Cerrar</button>
          </div>
        </div>
      )}
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col p-6 animate-in slide-in-from-right">
          ...
          <div className="flex flex-col gap-6 text-2xl font-black uppercase italic">
            <button onClick={() => { setCurrentView('workout'); setIsMenuOpen(false); }} className={`text-left ${currentView === 'workout' ? 'text-yellow-400' : 'text-zinc-600'}`}>Rutina</button>
            ...
            <button onClick={() => { setCurrentView('chat'); setIsMenuOpen(false); }} className={`text-left flex items-center gap-4 ${currentView === 'chat' ? 'text-yellow-400' : 'text-zinc-600'}`}>Chat {unreadCount > 0 && <span className="bg-red-500 text-white text-xs px-2 py-1 rounded-full font-sans not-italic">{unreadCount}</span>}</button>
            <button onClick={() => { setCurrentView('profile'); setIsMenuOpen(false); }} className={`text-left ${currentView === 'profile' ? 'text-yellow-400' : 'text-zinc-600'}`}>Perfil</button>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 -mt-6 pb-28 relative z-20">
        ...
        {currentView === 'workout' && (
          <div className="space-y-6">
            {!hasPaidMonth && (
              <div onClick={() => setShowPaymentModal(true)} className="bg-red-600 text-white p-4 rounded-2xl flex items-center justify-between shadow-lg animate-pulse cursor-pointer">
            ...
            <div className={`space-y-4 ${!hasPaidMonth ? 'opacity-20 pointer-events-none' : ''}`}>
                {dailySession.length > 0 ? dailySession.map((ex, exIdx) => (
                    <div key={exIdx} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 shadow-sm">
                      ...
                      {ex.rir && (
                        <div className="mb-4 bg-black/40 p-3 rounded-xl border border-zinc-800/50">
                          <div className="flex justify-between items-center mb-1.5"><span className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Intensidad Percibida</span><span className="text-[10px] text-black font-black bg-yellow-400 px-2 py-0.5 rounded">RIR {ex.rir}</span></div>
                          <div className="flex gap-1 h-2.5">{[5, 4, 3, 2, 1, 0].map((val, i) => (<div key={val} className={`flex-1 rounded-full transition-colors ${val >= parseInt(ex.rir) ? rirColors[i] : 'bg-zinc-800'}`}></div>))}</div>
                        </div>
                      )}

                      <div className="grid grid-cols-4 gap-2 mb-2 px-2 text-[9px] uppercase font-black text-zinc-500 text-center"><span>Serie</span><span>Objetivo</span><span>Reps</span><span>Peso</span></div>
                ...
                )) : <div className="text-center py-20 bg-zinc-900/30 rounded-[2rem] border border-dashed border-zinc-800 text-zinc-600 uppercase font-black text-sm tracking-widest">Descanso o sin rutina</div>}
            </div>
            
            {dailySession.length > 0 && !isSessionFinalized && hasPaidMonth && (
              <button onClick={handleFinishWorkout} className="w-full bg-green-500 text-black font-black py-5 rounded-[2rem] uppercase tracking-widest shadow-xl active:scale-95 transition-transform">Finalizar Entrenamiento</button>
            )}
            <div className="bg-zinc-900 rounded-[2rem] border border-zinc-800 p-6">
               <Calendar onChange={(d) => !hasPaidMonth ? setShowPaymentModal(true) : setDate(d)} value={date} className="react-calendar-custom-mini" tileClassName={({ date }) => allSessionsIds.includes(formatDateId(date)) ? 'has-workout' : null} />
            </div>
          </div>
        )}
        ...
        {currentView === 'chat' && (
          <div className="bg-zinc-900 rounded-[2rem] border border-zinc-800 flex flex-col h-[65vh] shadow-xl overflow-hidden animate-in fade-in">
            <div className="p-4 border-b border-zinc-800 bg-zinc-950/50 flex items-center gap-3 shrink-0"><MessageSquare className="text-yellow-400" size={24}/><h3 className="text-white font-bold uppercase tracking-widest text-sm">Chat con Coach</h3></div>
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-zinc-950/20 custom-scrollbar">
              {messages.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-zinc-600 opacity-50"><MessageSquare size={48} className="mb-4" /><p>Escribe tu primer mensaje.</p></div> : 
                messages.map(msg => (
                  <div key={msg.id} className={`flex ${msg.sender === 'student' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-4 rounded-2xl text-sm ${msg.sender === 'student' ? 'bg-yellow-400 text-black rounded-tr-none' : 'bg-zinc-800 text-white rounded-tl-none border border-zinc-700'}`}>{msg.text}<span className="block text-[10px] opacity-60 mt-2 text-right">{msg.createdAt?.toDate().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span></div>
                  </div>
                ))}
              <div ref={messagesEndRef} />
            </div>
            <form onSubmit={handleSendMessage} className="p-3 bg-zinc-950 border-t border-zinc-800 flex gap-2 shrink-0">
              <input type="text" placeholder="Escribe aquí..." className="flex-1 bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-white focus:border-yellow-400 outline-none text-sm" value={newMessage} onChange={e => setNewMessage(e.target.value)} /><button type="submit" disabled={!newMessage.trim()} className="bg-yellow-400 disabled:opacity-50 text-black p-3 rounded-xl"><Send size={20}/></button>
            </form>
          </div>
        )}
        ...
        {currentView === 'profile' && (
          <div className="space-y-6 animate-in fade-in">
             ...
             <div className="bg-zinc-900 rounded-[2rem] p-6 border border-zinc-800">
               <h3 className="text-white font-bold uppercase mb-6 flex items-center gap-2"><CreditCard size={20} className="text-yellow-400"/> Suscripción</h3>
               <div className="space-y-4">
                  <div className="flex justify-between items-center bg-black/40 p-5 rounded-2xl border border-zinc-800"><div><p className="text-[10px] text-zinc-500 uppercase font-black">Mes Actual</p><p className="font-bold text-lg uppercase">{date.toLocaleString('es-ES', { month: 'long' })}</p></div><span className={`text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-widest ${hasPaidMonth ? 'bg-green-500/20 text-green-500 border border-green-500/30' : 'bg-red-500/20 text-red-500 border border-red-500/30'}`}>{hasPaidMonth ? 'Al Día' : 'Pendiente'}</span></div>
                  <div className="bg-black/40 p-5 rounded-2xl border border-zinc-800"><p className="text-[10px] text-zinc-500 uppercase font-black mb-1">Alias Mercado Pago:</p><p className="font-mono font-bold text-yellow-400 text-lg select-all cursor-pointer">{trainerSettings.alias || 'SIN_ALIAS'}</p></div>
                  <button onClick={() => window.open('https://www.mercadopago.com.ar/', '_blank')} className="w-full bg-yellow-400 text-black font-black py-5 rounded-2xl uppercase tracking-widest flex justify-center items-center gap-2">Ir a Mercado Pago <ExternalLink size={20}/></button>
               </div>
             </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-zinc-950/80 backdrop-blur-xl border-t border-zinc-800 p-4 pb-8 flex justify-around items-center z-[40]">
        <button onClick={() => setCurrentView('workout')} className={`flex flex-col items-center gap-1 w-1/4 ${currentView === 'workout' ? 'text-yellow-400' : 'text-zinc-600'}`}><Dumbbell size={24}/><span className="text-[9px] font-black uppercase">Rutina</span></button>
        <button onClick={() => setCurrentView('community')} className={`flex flex-col items-center gap-1 w-1/4 ${currentView === 'community' ? 'text-yellow-400' : 'text-zinc-600'}`}><Users size={24}/><span className="text-[9px] font-black uppercase">Salón</span></button>
        <button onClick={() => setCurrentView('chat')} className={`flex flex-col items-center gap-1 w-1/4 relative ${currentView === 'chat' ? 'text-yellow-400' : 'text-zinc-600'}`}><div className="relative"><MessageSquare size={24}/>{unreadCount > 0 && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border border-black"></span></div><span className="text-[9px] font-black uppercase">Chat</span></button>
        <button onClick={() => setCurrentView('profile')} className={`flex flex-col items-center gap-1 w-1/4 ${currentView === 'profile' ? 'text-yellow-400' : 'text-zinc-600'}`}><User size={24}/><span className="text-[9px] font-black uppercase">Perfil</span></button>
      </nav>
    </div>
  );
}
